...
# # Get API key from .env file (OR use a direct API key)
# api_key = os.getenv("GOOGLE_API_KEY", "AIzaSyC0dM9uOL36CyroYDFxL4ua-CbeL92KyiU")  # Use your API key here
...
#     duration = data.get("duration", 100)  # Default to 30 days if not specified

#     if not topic:
#         return jsonify({"error": "Topic is required"}), 400

#     prompt_message = f"""
#     Generate a {duration}-day learning roadmap for {topic}.
#     Include detailed task descriptions and relevant quiz questions for each day.
...
#         print(f"AI Response: {response_text}")
#         return jsonify({"error": "Failed to decode the AI's response to JSON. Please try again."}), 500
